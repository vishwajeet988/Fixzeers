...
export async function recalculateReputation(professionalId: string) {
  const jobsResult = await query<any>(
    `SELECT
       ...
       COUNT(*) FILTER (
         WHERE status IN ('completed', 'customer_confirmed')
       ) AS completed_jobs
     FROM jobs
  ...
  const verifiedJobs =
    Number(jobsResult.rows[0]?.verified_jobs || 0);

  const totalJobs =
    Number(jobsResult.rows[0]?.total_jobs || 0);

  const completedJobs =
    Number(jobsResult.rows[0]?.completed_jobs || 0);
  ...
  const years =
    Number(profileResult.rows[0]?.years_experience || 0);

  const jobComponent =
    Math.min(verifiedJobs / 100, 1) * 40;
  ...
  const verificationComponent =
    Math.min(
      Number(verificationResult.rows[0]?.count || 0),
      1
    ) * 10;
